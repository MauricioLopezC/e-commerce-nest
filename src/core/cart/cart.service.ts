...
export class CartService {
  ...
  async findOne(userId: number) {
    ...
    const cartTotal = cartItems.reduce((previous: Prisma.Decimal, current) => (
  ...
  async checkDiscounts(userId: number): Promise<CalculateDiscountsResponse> {
    ...
    const calculateDiscounts = await this.discountsService.calculateDiscounts(cart.id, total, false)
    const discounts = await this.prisma.discount.findMany({
      where: {
        id: {
          in: calculateDiscounts.appliedDiscounts.map(item => item.discountId)
        }
      }
    })
    //join appliedDisocuntsArray with discountName
    const appliedDiscounts = calculateDiscounts.appliedDiscounts.map((item) => {
      const discount: Discount = discounts.find((discount) => discount.id === item.discountId)
      return (
        {
          ...item,
